...
export const INSIGHTS_KEPT = [
  ...
  },
  {
    ...
    impact: "Live Nation's FY2025 10-K reports 346 million fee-bearing tickets on US$37.1 billion [OFFICIAL] of fee-bearing GTV — a statutory SEC filing, deep-linked in verify/us-stats.md; a sanity bound on per-ticket fee economics at gate G1. (Closure pass, F-04: this figure carried the label “Audited filing”, which this audit invented for it and which is in no sanctioned marker set; it now carries [OFFICIAL], and the source's trust tier is unchanged — financial_rebuild.md §B.2 still grades an audited filing one rung below an official statistic.) Benchmark use only: audited filings may never serve as cost comparators for an A$830 entity, and no revenue figure for this programme may be derived from them.",
...
export const MAP_CALLOUTS = [
  ...
  { title: 'UK Live Entertainment Scale', body: 'UK live-music market size is [UNKNOWN] on this site. The consumer-spend figure carried here on earlier passes — correctly re-attributed from DCMS to LIVE, the trade federation, but then published under the label “Trade-body estimate” — is withdrawn on the closure pass (F-04), together with its AUD conversion: that label was minted by this audit, not drawn from the sanctioned marker set, and no sanctioned marker fits a trade body’s own commissioned estimate. It is not republished under a new label. Nothing downstream moves — it was market context only, never a TAM, and no market page on this site publishes one. What stands for the UK is the official statistic: 90.6% of adults in England engaged with the arts in 2024/25 — a broad measure including digital engagement, down from 91.4% in 2023/24.', source: 'DCMS Participation Survey 2024/25' },
