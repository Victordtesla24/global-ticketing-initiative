// The revenue model — the identity and the variables that resolve it.

export const REVENUE_IDENTITY = {
  intro:
    'No take rate, customer acquisition cost, average transaction value or repeat-purchase figure has been verified against first-party data. The revenue statement for Australia is therefore the arithmetic identity itself:',
  formulaGross: 'Gross AU pilot revenue / yr = E × T × ATV × f',
  formulaShare: 'Ticketalay’s share of that gross = the above × s',
  outro:
    'Once the partnership terms, the contracted supply and the primary demand study have each been delivered, every variable on the right-hand side becomes a fact and the identity resolves per event and per gate — never as a single cumulative headline.',
};
...
export const IDENTITY_VARIABLES: IdentityVariable[] = [
  {
    ...
    confirms:
      'Commercial lead, through at least three signed, dated pilot-event agreements or letters of intent. E is then the contracted count, not a target.',
    when: 'Gate G1',
  },
  {
    symbol: 'T',
