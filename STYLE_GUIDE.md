...
| `ComplianceLink` | `section: ComplianceLinkKey`, `className?` | The legal footing under a prototype section: the point that section raises, and a link to the matching anchor on `/compliance`. Keys are typed in `lib/data/compliance.ts`, so a wrong key fails the build rather than rendering a dead link. |
